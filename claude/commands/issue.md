...
- If the line has a `→plan:[[…]]` wikilink (or a legacy `[[PLAN …]]` link), the plan is the body: goal, files, steps, edge cases, acceptance criteria, rendered as markdown with wikilinks converted to plain text (GitHub cannot resolve them).
