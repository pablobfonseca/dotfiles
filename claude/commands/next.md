...
1. If no IDs were given, or the user asks you to "pick the best ones", "promote the top 3" or anything that delegates the choice: refuse plainly, and list what `/queue` last proposed (`claudeos queue dump <project>` → `proposed`). This is the one lane an agent never fills on its own judgment.
