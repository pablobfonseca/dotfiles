...
7. **Stamp the queue line.** Size the item from the written plan on the vault scale (`XS`, `S`, `M`, `L`, the shape's own size for a short plan, or the size the brainstorm settled for a full one) — this replaces the groom-time guess with what the plan proved. Decide `#claude`: set it when the written plan carries only `/implement-plan`'s default stop-and-ask triggers and leaves no decision open (a full plan with no item-specific stop trigger, or an unattended `--auto` run that stopped before step 6 never reaches this); clear a groom-time `#claude` when it does not (an item-specific stop trigger, or a short plan that hands a decision to the executor). `model:` stays an independent choice — do not tie it to this. Stamp plan, size and `#claude`/`#claude` clear in the one call: `claudeos queue mark <project> <qN> --plan '[[<project>/plans/YYYY-MM-DD-<topic>]]' --size <XS|S|M|L> [--tag claude | --untag claude]` — the tool inserts each token in the line's canonical position. Print one line naming the old and new size and whether `#claude` was set, cleared, or left as-is: `Sized <old> → <new>, #claude <set|cleared|unchanged>.` Run the same `mark` for every line the user bundled in step 5, but with `--plan` only — a bundled line rides the primary item's plan and keeps its own size and tags; the plan's `queue_item:` stays the primary line only.
